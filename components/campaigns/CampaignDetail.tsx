...
const CampaignDetail: React.FC<CampaignDetailProps> = ({ campaign, company }) => {
    ...
    const genders = () => {
        const result = gends.reduce((res, gend) => {
            if (campaign.genders.includes(gend.value as Gender)) return (res = `${res} - ${gend.label}`);
            else return res;
        }, '');
    ...
    const locations = () => {
        const result = locats.reduce((res, location) => {
            if (campaign.locations.includes(location.value as Location)) return (res = `${res} - ${location.label}`);
            else return res;
        }, '');
        return result.slice(2);
    };
        ...
        <>
                ...
                <div className="sm:flex sm:flex-[1] gap-8 items-center">
                    <Image
                        src={
                            campaign.image
                                ? campaign.image
                                : 'https://res.cloudinary.com/dngl8ihk7/image/upload/v1686058298/jfrql1oot1iyatc63ctz.jpg'
                        }
                        alt="img"
                        width={100}
                        height={100}
                    ...
                    <div className="mt-4 sm:mt-0">
                        <h3 className="text-[#00b14f] text-lg sm:text-xl font-bold text-center">{campaign?.title}</h3>
                        <Link
                        ...
                        <div className="flex gap-2 mt-2 sm:mt-4 justify-center sm:justify-start items-center">
                ...
                <div className="flex flex-col sm:mr-4 mx-auto items-center md:items-baseline">
                    {myKol.kol && (
                        <>
                            <button
                                onClick={sendEmail}
                                className="w-[150px] mt-4 md:mt-0 md:w-auto border-[#00b14f] border-2 bg-[#00b14f] text-white py-2 px-4 rounded-lg text-sm flex items-center"
                            >
                                <FaRegPaperPlane />
            ...
            <div className="mt-4 bg-white shadow-lg  border-2 py-10 px-4 md:px-8 rounded-xl">
                ...
                <div>
                    ...
                    <div>{parse(campaign.description)}</div>
                </div>
                <div className="">
                    <p className="my-4 font-bold text-md">- Quyền lợi:</p>
                    <div>{parse(campaign.benefit)}</div>
                </div>
                {myKol.kol && (
                    <div className="mt-8 flex">
                        <button
                            onClick={sendEmail}
                            className="mr-4 border-[#00b14f] border-2 bg-[#00b14f] text-white py-2 px-4 rounded-lg text-sm flex items-center"
                        >
                            <FaRegPaperPlane />
